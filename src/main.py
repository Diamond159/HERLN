    ...
    with open(path+'ranks_filter.txt', 'w') as f:
        for ranks in ranks_filter:
            for r in ranks.cpu().numpy().tolist():
                f.writelines(str(r))
                f.writelines('\n')
    mrr_raw = utils.stat_ranks(ranks_raw, "raw_ent", verbose=False)
    mrr_filter = utils.stat_ranks(ranks_filter, "filter_ent", verbose=False)
    mrr_unaware = utils.stat_ranks(ranks_unaware, "unaware_ent", verbose=False)
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    model = RecurrentRGCN(args.decoder,
                        ...
                        gpu = args.gpu,
                                                ...
                                                use_relation_dynamics=getattr(args, 'use_relation_dynamics', False),
                                                use_copy_generation=getattr(args, 'use_copy_generation', False),
                                                copy_gen_alpha=getattr(args, 'copy_gen_alpha', 0.5))
